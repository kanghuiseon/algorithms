...
## 1700. 멀티탭 스케줄링 (골드 1)
### 구현
1. 현재 멀티탭에 꽂혀있으면 continue
2. 멀티탭에 비어있는 곳이 있으면 꽂고 continue
3. 멀티탭에 비어있는 곳이 없다면, 앞으로 한번도 사용하지 않을 플러기 또는 제일 나중에 사용할 플러그를 찾아서 그 자리에 꽂아준다.

## 10775. 공항 (골드 2)
비행기 i는 1부터 gi까지의 게이트 중에서 비어있는 곳에 도킹할 수 있다.
gi부터 도킹하는 건 알겠는데, gi부터 1씩 빼면서 확인하는 경우에 계속 시간초과가 나서 어려웠다.

Map을 하나 만들어서 해당 게이트(key)에 넣는 경우에 비어있는 곳을 value로 하고 value부터 -1씩 빼면서 확인했더니 시간초과가 해결됐다.

# 1781. 컵라면 (골드 2)
priority_queue를 min heap으로 사용한다.

데드라인, 컵라면 수를 오름차순으로 정렬한 후에, 

pq에 넣고 pq의 크기를 이때까지 지난 시간이라고 생각한다.

만약 pq의 크기가 현재 데드라인보다 크다면 pq.pop을 통해서 가장 작은 수의 컵라면을 빼준다. 

# 1941. 소문난 칠공주
## 구현
1. dfs로 25명 중에서 7명 조합을 만든다.
2. 그 조합에서 다솜이가 4명이상인 경우에만 본다.
3. 그 조합이 연결되어있는지를 bfs를 통해서 본다.

* dfs로는 십자가모양을 볼 수가 없다….
